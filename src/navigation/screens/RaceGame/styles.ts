...
export function getRaceGameSizeStyles(width: number, height: number) {
  const LANE_WIDTH = width * 0.8 / LANE_COUNT;
  const GAME_HEIGHT = height * 0.6;
  return {
    track: { width: width * 0.8, height: GAME_HEIGHT },
    player: { width: LANE_WIDTH * 0.8 },
    enemy: { width: LANE_WIDTH * 0.8 },
  };
}
...
export const styles = StyleSheet.create({
  ...
  track: { backgroundColor: '#050505', borderWidth: 2, borderColor: '#333', flexDirection: 'row', overflow: 'hidden' },
  laneLine: { flex: 1, borderRightWidth: 1, borderRightColor: '#222', borderStyle: 'dashed' },
  player: { position: 'absolute', bottom: 20, height: 60, alignItems: 'center', justifyContent: 'center' },
  ...
  enemy: { position: 'absolute', height: 50, alignItems: 'center' },
  ...
  overlay: { ...StyleSheet.absoluteFill, backgroundColor: 'rgba(0,0,0,0.8)', justifyContent: 'center', alignItems: 'center', zIndex: 10 },
